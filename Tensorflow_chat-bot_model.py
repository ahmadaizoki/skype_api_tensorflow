...
for doc in documents:
    ...
    bag = []
    # lister les mots tokenizé pour les patterns
    pattern_words = doc[0]
    # reserve chaque mot
...
# Definer la module et télécharger tensorboard
    ...
    for s in sentence_words:
        for i,w in enumerate(words):
            if w == s:
                bag[i] = 1
                if show_details:
                    print ("trouvé dans la sac: %s" % w)

    return(np.array(bag))

######################################################

p = bow("ton magasin ouverte aujourd'hui?", words)
print (p)
print (classes)
